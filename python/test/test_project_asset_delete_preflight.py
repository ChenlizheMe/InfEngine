...
def test_project_wait_for_asset_timeout_is_a_failed_observation(tmp_path, monkeypatch):
    from Infernux.mcp.tools import project as project_tools
    ...
    class Queue:
        ...
        @staticmethod
        def run_sync(_name, callback, timeout_ms=0):
            del timeout_ms
            return callback()

    assets = tmp_path / "Assets"
    assets.mkdir()
    monkeypatch.setattr(project_tools, "get_asset_database", lambda: Database())
    monkeypatch.setattr(project_tools, "MainThreadCommandQueue", Queue)
    mcp = FakeMcp()
    project_tools.register_project_tools(mcp, str(tmp_path))

    response = mcp.tools["project_wait_for_asset"](
        "Assets/Missing.mat",
        exists=True,
    ...
    assert response["error"]["code"] == "ASSET_WAIT_TIMEOUT"
    assert response["explain"]["state"]["settled"] is False
