"""NumPy AOT candidate backend for validated particle Kernel SSA programs."""
...
from .hir import ParticleProgramHIR
...
class NumpyParticleCompiler:
    ...
    def compile(
        self,
        hir: ParticleProgramHIR,
        kernel: ParticleKernelProgram,
    ) -> NumpyParticleProgram:
        if hir.behavior_hash != kernel.source_behavior_hash:
            raise NumpyParticleBackendError("Particle HIR and Kernel IR behavior hashes differ")
        hir_by_id = {emitter.stable_id: emitter for emitter in hir.emitters}
        if tuple(hir_by_id) != tuple(emitter.stable_id for emitter in kernel.emitters):
            raise NumpyParticleBackendError("Particle HIR and Kernel IR emitter schedules differ")
        programs = []
        for emitter in kernel.emitters:
            settings = hir_by_id[emitter.stable_id].settings
            programs.append(
