...
class CollisionEnterOnlyProbe(InxComponent):
    ...
    // The production runtime is a Python-owned DLL and is reclaimed together
    // with its host process. Avoid running the embedding-only CRT/DLL teardown
    // sequence, which is not a supported ownership path and can fast-fail on
    // Windows after every assertion and local Python reference has succeeded.
